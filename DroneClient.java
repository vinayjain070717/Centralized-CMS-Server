...
class DroneClient
{
	...
	{
		DroneMainClient mainClient=new DroneMainClient();
	}
}
class StatusTimer extends TimerTask
{
	WebSocketClient webSocketClient;
	public StatusTimer(WebSocketClient webSocketClient)
	{
	...
	public void run()
	{
		...
		String request="\"typeId:1,id:avx8,dateTime:2020-01-07T20:18:50.384Z,message:StatusNotificationRequest,payload:";
		request+="{\"distanceTravelled\":50,\"latitude\":20.7972,\"longitude\":76.9922,\"altitude\":25,\"speed\":7,\"remainingFlightTime\":12,\"timeElapsed\":10,\"airSpeed\":40}\"";
		this.webSocketClient.send(request);
	}
}
class DroneMainClient extends JFrame implements ActionListener
{
		...
		if(ev.getSource()==connect)
		{
			...
			webSocketClient=new WebSocketClient(URI.create(urlTextField.getText().trim())){
				public void onOpen(ServerHandshake sh)
				{
					...
					ta.append("Connected\n");
					// TimerTask timerTask=new StatusTimer(webSocketClient);
					// java.util.Timer timer=new java.util.Timer();
					// timer.schedule(timerTask,1000,new StatusNotificationRequest().getInterval());
				...
				{
					System.out.println("Message arrived At Drone");
